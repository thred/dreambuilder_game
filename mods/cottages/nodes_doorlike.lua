...
minetest.register_node("cottages:window_shutter_open", {
		...
		drop = "cottages:window_shutter_closed",
                on_rightclick = function(pos, node, puncher)
                    minetest.add_node(pos, {name = "cottages:window_shutter_closed", param2 = node.param2})
...
minetest.register_node("cottages:window_shutter_closed", {
		...
		selection_box = {
			...
			fixed = {
                ...
                on_rightclick = function(pos, node, puncher)
                    minetest.add_node(pos, {name = "cottages:window_shutter_open", param2 = node.param2})
...
minetest.register_abm({
   ...
   action = function(pos)
        ...
        if( not(minetest.get_timeofday() < 0.2 or minetest.get_timeofday() > 0.805)) then
           ...
           minetest.add_node(pos, {name = "cottages:window_shutter_open", param2 = old_node.param2})
           cottages_window_sutter_operate( pos, "cottages:window_shutter_closed", "cottages:window_shutter_open" );
       end
   end
})
...
minetest.register_abm({
   ...
   action = function(pos)
        ...
        if( minetest.get_timeofday() < 0.2 or minetest.get_timeofday() > 0.805) then
           local old_node = minetest.get_node( pos );
           minetest.add_node(pos, {name = "cottages:window_shutter_closed", param2 = old_node.param2})
           cottages_window_sutter_operate( pos, "cottages:window_shutter_open", "cottages:window_shutter_closed" );
        end
   end
})
		...
		selection_box = {
			...
			fixed = {
                ...
                on_rightclick = function(pos, node, puncher)
                    ...
                    minetest.add_node(pos, {name = "cottages:half_door", param2 = param2})
                    ...
                    if( node2 ~= nil and node2.name == node.name and node2.param2==node.param2) then
                       minetest.add_node( {x=pos.x,y=(pos.y+1),z=pos.z}, {name = "cottages:half_door", param2 = param2})
                    end
                end,
		is_ground_content = false,
})
		...
		selection_box = {
			...
			fixed = {
                ...
                on_rightclick = function(pos, node, puncher)
                    ...
                    minetest.add_node(pos, {name = "cottages:half_door_inverted", param2 = param2})
                    -- open upper parts of this door (if there are any)
                    if( node2 ~= nil and node2.name == node.name and node2.param2==node.param2) then
                       minetest.add_node( {x=pos.x,y=(pos.y+1),z=pos.z}, {name = "cottages:half_door_inverted", param2 = param2})
                    end
                end,
		is_ground_content = false,
})
...
minetest.register_node("cottages:gate_closed", {
		...
		selection_box = {
			...
			fixed = {
                ...
                on_rightclick = function(pos, node, puncher)
                    minetest.add_node(pos, {name = "cottages:gate_open", param2 = node.param2})
                end,
		is_ground_content = false,
})
...
minetest.register_node("cottages:gate_open", {
		...
		selection_box = {
			...
			fixed = {
                ...
                on_rightclick = function(pos, node, puncher)
                    minetest.add_node(pos, {name = "cottages:gate_closed", param2 = node.param2})
                end,
		is_ground_content = false,
})
...
--                                {-0.5, -0.55, 0.3, 0.3, -0.45, 0.5},
                ...
                on_rightclick = function(pos, node, puncher)
                    ...
                    minetest.add_node(pos, {name = node.name, param2 = new_facedirs[ node.param2+1 ]})
                end,
		is_ground_content = false,
	})
